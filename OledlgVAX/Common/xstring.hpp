...
namespace std {
    ...
    public:
        ...
        xstring(xstring_extension, const string& str, DWORD CodePage = CP_ACP) {
            ...
            len = MultiByteToWideChar(CodePage, 0, str.c_str(), -1, (LPWSTR)data(), len);
            if (len == 0) {
                auto err = GetLastError();
                throw std::system_error(err, std::system_category());
            }
        ...
        xstring(xstring_extension, const char* lpstr, DWORD CodePage = CP_ACP) {
            ...
            len = MultiByteToWideChar(CodePage, 0, lpstr, -1, (LPWSTR)data(), len);
            if (len == 0) {
                auto err = GetLastError();
                throw std::system_error(err, std::system_category());
            }
    ...
    public:
        ...
        xstring(xstring_extension, const string& str, DWORD CodePage = CP_ACP) {
            if (CodePage == CP_ACP || CodePage == GetACP()) {
                ...
                len = MultiByteToWideChar(CodePage, 0, str.c_str(), -1, (LPWSTR)wstr.data(), len);
                if (len == 0) {
                    auto err = GetLastError();
                    throw std::system_error(err, std::system_category());
                }
                ...
                len = WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), -1, (LPSTR)data(), len, NULL, NULL);
                if (len == 0) {
                    auto err = GetLastError();
                    throw std::system_error(err, std::system_category());
                }
        ...
        xstring(xstring_extension, const char* lpstr, DWORD CodePage = CP_ACP) {
            if (CodePage == CP_ACP || CodePage == GetACP()) {
                ...
                len = MultiByteToWideChar(CodePage, 0, lpstr, -1, (LPWSTR)wstr.data(), len);
                if (len == 0) {
                    auto err = GetLastError();
                    throw std::system_error(err, std::system_category());
                }
        ...
        xstring(xstring_extension, const wchar_t* lpwstr) {
            ...
            len = WideCharToMultiByte(CP_ACP, 0, lpwstr, -1, (LPSTR)data(), len, NULL, NULL);
            if (len == 0) {
                auto err = GetLastError();
                throw std::system_error(err, std::system_category());
            }
        ...
        std::string explicit_string(DWORD CodePage = CP_ACP) const {
            ...
            len = WideCharToMultiByte(CodePage, 0, c_str(), -1, (LPSTR)str.data(), len, NULL, NULL);
            if (len == 0) {
                auto err = GetLastError();
                throw std::system_error(err, std::system_category());
            }
            ...
            if (CodePage == CP_ACP || CodePage == GetACP()) {
                ...
                len = MultiByteToWideChar(CP_ACP, 0, c_str(), -1, (LPWSTR)wstr.data(), len);
                if (len == 0) {
                    auto err = GetLastError();
                    throw std::system_error(err, std::system_category());
                }
                ...
                len = WideCharToMultiByte(CodePage, 0, wstr.c_str(), -1, (LPSTR)str.data(), len, NULL, NULL);
                if (len == 0) {
                    auto err = GetLastError();
                    throw std::system_error(err, std::system_category());
                }
