...
mpz_rootrem (mpz_t x, mpz_t r, const mpz_t y, unsigned long z)
{
  ...
  mpz_init (u);
  mpz_init (t);
  mpz_setbit (t,(mp_bitcnt_t)( mpz_sizeinbase (y, 2) / z + 1));
	       ...
	       mp_bitcnt_t b0, const mpz_t n)
{
  ...
  for (bs = (mp_bitcnt_t)(mpz_sizeinbase (n, 2) - 1); --bs >= b0;)
    {
      /* U_{2k} <- U_k * V_k */
      mpz_mul (U, U, V);
      /* V_{2k} <- V_k ^ 2 - 2Q^k */
...
mpz_set_str (mpz_t r, const char *sp, int base)
{
  ...
  if (bits > 0)
    {
      alloc = (mp_size_t)((dn * bits + GMP_LIMB_BITS - 1) / GMP_LIMB_BITS);
  ...
  else
    {
      struct mpn_base_info info;
      mpn_get_base_info (&info, base);
      alloc = (mp_size_t)((dn + info.exp - 1) / info.exp);
      rp = MPZ_REALLOC (r, alloc);
	    ...
	    size_t nails, const void *src)
{
  ...
  rn = (mp_size_t)((size * count + sizeof(mp_limb_t) - 1) / sizeof(mp_limb_t));
  rp = MPZ_REALLOC (r, rn);
